...
template <typename FF, int CRV_A, typename Grp> struct ec_jac {
  ...
  // static constexpr int ELT_LIMBS_ = FF::ELT_LIMBS;
  // template <int ELT_LIMBS_ = ELT_LIMBS>
  // template <int ELT_LIMBS_>
  __device__ static void load_affine(ec_jac &P, const var *mem) {
    FF::load(P.x, mem);
    FF::load(P.y, mem + FF::DEGREE * FF::ELT_LIMBS);
    FF::set_one(P.z);
  ...
  // template <int ELT_LIMBS_ = ELT_LIMBS>
  // template <int ELT_LIMBS_>
  __device__ static void load_jac(ec_jac &P, const var *mem) {
    FF::load(P.x, mem);
    FF::load(P.y, mem + FF::DEGREE * FF::ELT_LIMBS);
    FF::load(P.z, mem + 2 * FF::DEGREE * FF::ELT_LIMBS);
  }

  __device__ static int is_affine(const ec_jac &P) {
    FF one;
    FF::set_one(one);
    return FF::are_equal(P.z, one);
  }

  __device__ static int are_equal(const ec_jac &P, const ec_jac &Q) {
    ...
    store_affine(var *mem, const ec_jac &P) {
        ...
        FF::store(mem + FF::DEGREE * FF::ELT_LIMBS, aff_y);
    }
#endif

  __device__ static void store_jac(var *mem, ec_jac &P) {
    FF::store(mem, P.x);
    FF::store(mem + FF::DEGREE * FF::ELT_LIMBS, P.y);
    FF::store(mem + 2 * FF::DEGREE * FF::ELT_LIMBS, P.z);
  }

  __device__ static void set_zero(ec_jac &P) {
    FF::set_one(P.x);
    FF::set_one(P.y);
    FF::set_zero(P.z);
  }

  __device__ static int is_zero(const ec_jac &P) { return FF::is_zero(P.z); }
  ...
  __device__ static void mixed_add(ec_jac &R, const ec_jac &P,
                                   const ec_jac &Q) {
  ...
  __device__ static void add_unsafe(ec_jac &R, const ec_jac &P,
                                    const ec_jac &Q) {
  ...
  __device__ static void add(ec_jac &R, const ec_jac &P, const ec_jac &Q) {
  ...
  __device__ static void dbl(ec_jac &R, const ec_jac &P) {
  ...
  __device__ __forceinline__ static void mul_2exp(int EXP, ec_jac &R, const ec_jac &P) {
    dbl(R, P);
#pragma unroll
    for (int k = 1; k < EXP; ++k)
      dbl(R, R);
  ...
  __device__ static void neg(ec_jac &R, const ec_jac &P) {
    R.x = P.x;
    FF::neg(R.y, P.y);
    R.z = P.z;
  }

  __device__ static void mul(ec_jac &R, const var &n, const ec_jac &P) {
...
typedef ec_jac<Fq_type, 2, Fr_type> G1_type;
typedef ec_jac<Fq2_type, 2 * 13, Fr_type> G2_type;

// typedef ec_jac<Fp_type, 2, Fr_type_> G1_type;
// typedef ec_jac<Fp2_type, 2 * 13, Fr_type_> G2_type;

// #ifdef MNT4753
// typedef ec_jac<Fp_MNT6, 11, Fp_type> ECp_MNT6;
// typedef ec_jac<Fp3_MNT6, -1, Fp_type> ECp3_MNT6;
// #endif