...
ec_multiexp_straus(var *out, const var *multiples_, const var *scalars_, size_t N)
{
    ...
    if (idx < n) {
        ...
        size_t R = (idx < n - 1) ? RR : ((N % RR) ? (N % RR) : RR);
...
#else
    auto tmp = (var *)malloc(total_bytes);
    if (fread((void *)tmp, total_bytes, 1, inputs) < 1) {
        fprintf(stderr, "Failed to read scalars\n");
        abort();
    }
    cudaMemcpy(mem.get(), tmp, total_bytes, cudaMemcpyHostToDevice);
    free(tmp);
#endif
    return mem;
}

var_ptr convert_scalars_format(mnt4753_libsnark::vector_Fr *scalar_start, size_t length) {
    static constexpr size_t scalar_bytes = ELT_BYTES;
    size_t total_bytes = length * scalar_bytes;
    auto mem = allocate_memory(total_bytes);
#ifdef USE_UNIFIED_MEMORY
    var *H_ = mem.get();
    mnt4753_libsnark::convert_Fr(scalar_start, H_, length);
#else
    auto tmp = (var *)malloc(total_bytes);
    var *H_ = tmp;
    mnt4753_libsnark::convert_Fr(scalar_start, H_, length);
...
var_ptr convert_scalars_format(mnt6753_libsnark::vector_Fr *scalar_start, size_t length) {
    static constexpr size_t scalar_bytes = ELT_BYTES;
    size_t total_bytes = length * scalar_bytes;
    auto mem = allocate_memory(total_bytes);
    var *H_ = mem.get();
    mnt6753_libsnark::convert_Fr(scalar_start, H_, length);
    return mem;
}
...
#else
    auto tmp = (var *)malloc(total_aff_bytes);
    if (fread((void *)tmp, total_aff_bytes, 1, inputs) < 1) {
        fprintf(stderr, "Failed to read all curve poinst\n");
        abort();
    }
    cudaMemcpy(mem.get(), tmp, total_aff_bytes, cudaMemcpyHostToDevice);
    free(tmp);
#endif
    return mem;
}
