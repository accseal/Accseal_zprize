        ...
        const char *preprocessed_path)
{
    ...
    // auto H_mults = load_points_affine<ECp>(((1U << C) - 1)*(m - 1), preprocessed_file);
    // auto out_H = allocate_memory(space * ECpe::NELTS * ELT_BYTES);
    ...
    // read_size_t(params_file);
    // read_size_t(params_file);
    // auto A1_mults = load_points<ECp>(m+1, params_file);;
    // auto B1_mults = load_points<ECp>(m+1, params_file);
    // auto B2_mults = load_points<ECpe>(m+1, params_file);
    // auto L_mults = load_points<ECp>(m-1, params_file);
    // fclose(params_file);
    ...
    auto t_multiexp = t;
    G1 *evaluation_At = B::multiexp_G1(B::input_w(inputs), B::params_A(params), m + 1);
    print_time(t, "multiexp A done");
    G1 *evaluation_Bt1 = B::multiexp_G1(B::input_w(inputs), B::params_B1(params), m + 1);
    print_time(t, "multiexp B1 done");
    G1 *evaluation_Lt = B::multiexp_G1(B::input_w(inputs), B::params_L(params), m - 1);
    print_time(t, "multiexp L done");
    G1 *evaluation_Ht = B::multiexp_G1(coefficients_for_H, H, d);
    print_time(t, "multiexp H done");
    G2 *evaluation_Bt2 = B::multiexp_G2(B::input_w(inputs), B::params_B2(params), m + 1);
    print_time(t, "multiexp B2 done");
    print_time(t_multiexp, "multiexp done");
    ...
    // ec_reduce_straus<ECp, C, R>(sH, out_H.get(), H_mults.get(), coefficients_for_H->data.get(), m - 1);
...
template <typename B>
void run_prover_for_application(
        const char *params_path,
        const char *input_path)
{
    B::init_public_params();
    ...
    // size_t d = read_size_t(params_file);
    // size_t m = read_size_t(params_file);
    size_t a = read_size_t(params_file);
    size_t b = read_size_t(params_file);    
    size_t l = read_size_t(params_file);
    size_t h = read_size_t(params_file);
    rewind(params_file);

    printf("a = %zu, b = %zu, l = %zu, h = %zu\n", a, b, l ,h);
    ...
    auto params = B::read_params(params_file, a, b, l, h);
    fclose(params_file);
    print_time(t, "load params");
    ...
    // auto w_ = load_scalars(m + 1, inputs_file);
    // rewind(inputs_file);
    auto inputs = B::read_input(inputs_file, a, b, l, h);
    fclose(inputs_file);
    print_time(t, "load inputs");
    ...
    // auto H = B::params_H(params);
    auto coefficients_for_H =
        compute_H<B>(h, B::input_ca(inputs), B::input_cb(inputs), B::input_cc(inputs));
    print_time(t, "compute H done");
    auto t_multiexp = t;
    G1 *evaluation_At = B::multiexp_G1(B::input_a(inputs), B::params_A(params), a);
    print_time(t, "multiexp A done");
    G1 *evaluation_Bt1 = B::multiexp_G1(B::input_b(inputs), B::params_B1(params), b);
    print_time(t, "multiexp B1 done");
    G1 *evaluation_Lt = B::multiexp_G1(B::input_l(inputs), B::params_L(params), l);
    print_time(t, "multiexp L done");
    G1 *evaluation_Ht = B::multiexp_G1(coefficients_for_H, B::params_H(params), h);
    print_time(t, "multiexp H done");
    G2 *evaluation_Bt2 = B::multiexp_G2(B::input_b(inputs), B::params_B2(params), b);
    print_time(t, "multiexp B2 done");
    print_time(t_multiexp, "multiexp done");
...
int main(int argc, char **argv) {
  ...
  if (mode == "compute") {
      ...
      if (curve == "MNT4753") {
          run_prover_for_application<mnt4753_libsnark>(params_path, input_path);
      } else if (curve == "MNT6753") {
        run_prover_for_application<mnt6753_libsnark>(params_path, input_path);
      }
  }
