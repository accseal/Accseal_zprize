...
template<typename ppT>
class groth16_parameters_application {
public:
  size_t a;
  size_t b;
  size_t l;
  ...
  groth16_parameters_application(const char* path) {
    FILE* params = fopen(path, "r");
    a = read_size_t(params);
    b = read_size_t(params);
    l = read_size_t(params);
    h = read_size_t(params);
...
template <typename ppT>
void print_Fr(size_t d, const std::vector<Fr<ppT>> *a) {
  for (size_t i = 0; i < d; ++i) {
    auto x = a->at(i);
    for (int j = 0; j < 12; ++j) {
      std::cout << x.mont_repr.data[j] << " \n"[j == 11];
    }
  }
  std::cout << std::flush;
}
    ...
    const char* output_path)
{
    ...
    print_time(t, "load inputs and params done");
    auto t_proof = t;
    ...
    G1<ppT> evaluation_Lt = multiexp<G1<ppT>, Fr<ppT>>(
        ...
        parameters.L.begin(),
        parameters.m - 1);
    print_time(t, "Multi-exponentiations done");
    ...
    print_time(beginning, "Total time from input to output: ");
...
template<typename ppT>
void output_g1_multiples_by_C(int C, int step, const std::vector<G1<ppT>> &vec, FILE *output) {
    // If vec = [P0, ..., Pn], then multiples holds an array
    //
    // [    P0, ...,     Pn,
    //     2^CP0, ...,    2^CPn,
    ...
    std::vector<G1<ppT>> multiples;
    size_t len = vec.size();
    size_t win_nums = (753 + C - 1) / C;
    size_t step_num = (win_nums + step - 1) / step;
    multiples.resize(len * (step_num));
    std::copy(vec.begin(), vec.end(), multiples.begin());

    for (size_t i = 1; i < step_num; ++i) {
        size_t prev_row_offset = (i-1)*len;
        size_t curr_row_offset = i*len;
#ifdef MULTICORE
#pragma omp parallel for
#endif
        for (size_t j = 0; j < len; ++j){
            auto tmp = multiples[prev_row_offset + j];
            for(size_t k = 0; k < C * step; k++) {
                G1<ppT> ttmp = tmp + tmp;    
                tmp = ttmp;
            }
            multiples[curr_row_offset+j] = tmp;
        }
    }
    if (multiples.size() != step_num * len) {
        fprintf(stderr, "Broken preprocessing table: got %zu, expected %zu\n",
                multiples.size(), step_num * len);
        abort();
    }
    write_g1_vec<ppT>(output, multiples);
}
...
template<typename ppT>
void output_g2_multiples_by_C(int C, int step, const std::vector<G2<ppT>> &vec, FILE *output) {
    // If vec = [P0, ..., Pn], then multiples holds an array
    //
    // [    P0, ...,     Pn,
    //     2P0, ...,    2Pn,
...
#endif
        for (size_t j = 0; j < len; ++j) {
            multiples[curr_row_offset + j] = multiples[prev_row_offset + j];
            for (size_t k=0; k < C * step; k++) {
               multiples[curr_row_offset + j] = multiples[curr_row_offset + j] + multiples[curr_row_offset + j];
            }
        }
    }
    if (multiples.size() != step_num * len) {
...
template <typename ppT>
void run_preprocess(int C, const char *params_path, const char *output_path)
{
    ppT::init_public_params();
...
template <typename ppT>
void run_preprocess_for_pippenger(int C, int step, const char *params_path, const char *output_path)
{
    ppT::init_public_params();
    ...
    printf("Processing A(%lu)...\n", params.A.size());
    output_g1_multiples_by_C<ppT>(C, step, params.A, output);
    // printf("Processing B1(%lu)...\n", params.B1.size());
    // output_g1_multiples_by_C<ppT>(C, step, params.B1, output);
    // printf("Processing B2(%lu)...\n", params.B2.size());
    // output_g2_multiples_by_C<ppT>(C, step, params.B2, output);
    // printf("Processing L(%lu)...\n", params.L.size());
    // output_g1_multiples_by_C<ppT>(C, step, params.L, output);
    // printf("Processing H(%lu)...\n", params.H.size());
    // output_g1_multiples_by_C<ppT>(C, step, params.H, output);

    fclose(output);
}

template <typename ppT>
void run_preprocess_for_pippenger_h(int C, const char *params_path, const char *output_path)
{
    ppT::init_public_params();

    const groth16_parameters_application<ppT> params(params_path);
    size_t a = params.a, b =  params.b, l = params.l, h =  params.h;
    printf("a = %zu, b = %zu, l = %zu, h = %zu, C = %zu\n", a, b, l, h, C);
    FILE *output = fopen(output_path, "w");
    printf("Processing H(%lu)...\n", params.H.size());
    output_g1_multiples_by_C<ppT>(C, 1, params.H, output);
    fclose(output);
}
...
int main(int argc, const char * argv[])
{
  ...
  if (mode == "compute") {
      ...
      if (curve == "MNT4753") {
          run_preprocess<mnt4753_pp>(atoi(argv[5]), params_path, argv[4]);
      } else if (curve == "MNT6753") {
          run_preprocess<mnt6753_pp>(atoi(argv[5]), params_path, argv[4]);
      }
  } else if (mode == "preprocess_for_pippenger") {

      if (curve == "MNT4753") {
          run_preprocess_for_pippenger<mnt4753_pp>(atoi(argv[5]), atoi(argv[6]), params_path, argv[4]); // 第二个参数是跨步大小，默认是1，即没有跨度
      } else if (curve == "MNT6753") {
          run_preprocess_for_pippenger<mnt6753_pp>(atoi(argv[5]), atoi(argv[6]), params_path, argv[4]);
      }
  } else if (mode == "preprocess_for_pippenger_h") {

      if (curve == "MNT4753") {
          run_preprocess_for_pippenger_h<mnt4753_pp>(atoi(argv[5]), params_path, argv[4]);
      } else if (curve == "MNT6753") {
          run_preprocess_for_pippenger_h<mnt6753_pp>(atoi(argv[5]), params_path, argv[4]);
      }
  }
