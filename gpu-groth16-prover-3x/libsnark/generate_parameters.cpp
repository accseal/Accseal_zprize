...
int generate_paramaters(
    int log2_d,
    const char* params_path, const char* input_path)
{
    srand(time(NULL));
    setbuf(stdout, NULL);
...
int main(int argc, const char *argv[]) {
  if (3 >= argc) {
    std::cerr << "Usage: " << argv[0] << " log2n parameter_file input_file"
              << std::endl;
    return -1;
  }
  generate_paramaters<mnt4753_pp>(std::atoi(argv[1]), argv[2], argv[3]);
}
